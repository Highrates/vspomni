...
export default function YandexPvzMap({
  ...
  useEffect(() => {
    ...
    const withCoords = points.filter(
      (p) => p.position?.latitude != null && p.position?.longitude != null,
    )
    ...
    if (selectedPointId) {
      ...
      mapRef.current.setCenter(
        [withCoords[0].position!.latitude, withCoords[0].position!.longitude],
        14,
      )
    } else if (withCoords.length > 1 && !center) {
      const bounds = window.ymaps.util.bounds.fromPoints(
        withCoords.map((p: YandexPickupPoint) => [
          p.position!.latitude,
  ...
  return (
    <div className="space-y-2">
      ...
      >
        ...
        {scriptShouldLoad &&
          !mapLoading &&
          points.filter((p) => p.position?.latitude).length === 0 && (
