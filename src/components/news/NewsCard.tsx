...
export default function NewsCard({ date, title, shortText, imageUrl, articleUrl }: NewsCardProps) {
  ...
  return (
    <Link
      href={articleUrl || "/"}
      className={`w-full overflow-hidden grid ${hasImage ? 'grid-rows-[200px_auto] sm:grid-rows-[240px_auto] lg:grid-rows-1 lg:grid-cols-[200px_auto]' : 'grid-rows-1'} border border-borderdarkgrey rounded-xl hover:shadow-lg transition-shadow`}
    >
      {hasImage && (
      ...
      <div className="flex flex-col px-4 sm:px-5 md:px-6 py-5 sm:py-6 md:py-8 gap-4 sm:gap-5 md:gap-6">
        <div className="flex flex-col gap-1 sm:gap-1.5">
          ...
          <h5 className="text-lg sm:text-xl md:text-2xl text-black font-semibold select-none leading-tight line-clamp-2">
            {title}
          </h5>
        </div>
