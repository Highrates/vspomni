...
import { OrderDraftInput, OrderDraftNode, OrderDraftOrderCreateResponse } from '../../types/order'
...
export async function getProducts(): Promise<ProductNode[]> {
  const query = `
    query Products {
      products(first: 20, channel: "vspomni-site") {
        edges {
          node {
            id
            name
...
export async function getProductBySlug(slug: string): Promise<ProductNode> {
  ...
  const query = `
    query ProductBySlug($slug: String!) {
      product(slug: $slug, channel: "vspomni-site") {
        id
        name
        slug
        rating
...
export async function deleteAddress(
  ...
  if (data.addressDelete.errors.length > 0) {
    throw new Error(
      `Address deletion failed: ${data.addressDelete.errors.map((e) => e.message).join(', ')}`,
    )
  }
...
export async function fulfillOrder(orderId: string): Promise<OrderNode> {
  ...
  const result = await graphqlRequest<OrderFulfillMutationResponse>(
    mutation,
    variables,
  )
  ...
  if (result.orderFulfill.errors.length > 0) {
    ...
    throw new Error(
      `Order fulfillment failed: ${result.orderFulfill.errors.map((e) => e.message).join(', ')}`,
    )
  }
...
export async function createOrderDraftAPI(input: OrderDraftInput): Promise<OrderDraftNode | null> {
  const mutation = `
  ...
  const result = await graphqlRequest<OrderDraftOrderCreateResponse>(
    mutation,
    variables,
  );
